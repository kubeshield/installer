...
gen-crd-protos-%:
	...
	@docker run --rm                                     \
		...
		go-to-protobuf                                   \
			...
			--packages=-k8s.io/api/core/v1,kubeshield.dev/installer/apis/$(subst _,/,$*)
...
gen-values-schema:
	@yq r api/crds/installer.kubeshield.io_identityservers.yaml spec.validation.openAPIV3Schema.properties.spec > /tmp/identity-server-values.openapiv3_schema.yaml
	@yq d /tmp/identity-server-values.openapiv3_schema.yaml description > charts/identity-server/values.openapiv3_schema.yaml

.PHONY: gen-chart-doc
gen-chart-doc: gen-chart-doc-identity-server
